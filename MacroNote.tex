...
\begin{theorem}[变量的增长率]
    ...
    不断加减再做除的计算过程麻烦且无趣，
    现有一巧妙方法——变量增长率等于其自然对数关于时间$t$的一阶导数。\footnote{The use of natural logarithms for growth rates is convenient because the change in the log of a variable is a good approximation of its percentage change, especially for small changes. This simplifies many dynamic calculations in macroeconomics.}
    证明如下，
    \begin{equation*}
...
\begin{theorem}[Hamiltonian函数]
    ...
    以及共同状态变量(co-state variable) \, $\mu_t$. (State variables: 'stock' or 'level' of something important, e.g., capital.) (Control variables: 'choice' variables decided at each point in time, e.g., consumption.) (Co-state variables / shadow price: marginal value of relaxing a state variable's constraint.)}
    ...
    \end{align*}\footnote{The transversality condition (TVC) is a crucial boundary condition in infinite horizon optimization problems. Economically, it ensures that the value of the assets (like capital $k_t$) held at the very end (as time goes to infinity) is zero when discounted back to the present. This rules out scenarios where the agent accumulates debt indefinitely (Ponzi schemes) or over-accumulates assets beyond what's optimal for consumption over their infinite lifetime. It essentially imposes that all wealth is ultimately consumed or its value diminishes to zero at infinity.}
\end{theorem}
...
\begin{align}
  ...
  \end{aligned}
\end{align}\footnote{This core equation $\dot{k}_t = sf(k_t) - (n+g+\delta)k_t$ shows how the capital per effective worker, $k_t$, changes over time. The term $sf(k_t)$ is actual investment per effective worker (savings times output). The term $(n+g+\delta)k_t$ is the 'break-even' investment needed to keep $k_t$ constant: $n k_t$ for new workers, $g k_t$ for technological progress making existing capital less effective in relative terms, and $\delta k_t$ for replacing depreciated capital. If actual investment exceeds break-even investment, $k_t$ rises. If it's less, $k_t$ falls. The steady state $k^*$ is reached when $\dot{k}_t = 0$.}
根据推导结果，当
...
\begin{example}[$k^*$的黄金律值\footnote{The 'Golden Rule' level of capital, $k^*_{GR}$, refers to the steady-state capital stock per effective worker that maximizes consumption per effective worker. While higher capital always means higher output, it also requires higher investment (savings) to maintain. The Golden Rule strikes the optimal balance. If $k < k^*_{GR}$, increasing savings (and thus $k$) would increase future consumption. If $k > k^*_{GR}$ (dynamic inefficiency), reducing savings would decrease $k$ but increase consumption both now and in the future, meaning the economy has over-accumulated capital from a consumption perspective.}] \label{solowkgr}
    \mbox{} \\
    稳态下 
    \begin{equation*}
        sf(k^*) = (n+g+\delta)k^*.
...
由家庭效用最大化,得有效人均消费的动态方程。
        ...
        \end{align}\footnote{This is the Euler equation for consumption in the RCK model. It's a cornerstone of intertemporal optimization in macroeconomics. Here's a conceptual breakdown of its derivation: 1. Households maximize lifetime utility subject to a budget constraint. 2. A Hamiltonian is set up for this dynamic optimization problem. 3. First-order conditions are derived with respect to consumption ($c_t$) and capital ($k_t$). 4. The condition for $c_t$ typically relates marginal utility of consumption to the shadow price of capital ($\lambda_t$). 5. The condition for $k_t$ describes how the shadow price $\lambda_t$ evolves, relating it to the rate of return on capital and the discount rate. 6. By differentiating the first-order condition for $c_t$ with respect to time and substituting the evolution equation for $\lambda_t$, we eliminate $\lambda_t$ and arrive at this equation. It dictates that consumption growth depends on the difference between the market interest rate ($r_t$, or $f'(k_t)$) and the household's effective discount rate ($\rho + \theta g$), scaled by the intertemporal elasticity of substitution ($1/\theta$). If $r_t > \rho + \theta g$, households delay consumption, leading to positive consumption growth.}
...
若我们将初始$k_0$与$c_0$值一一建立联系，则得到图~\ref{ramseyr5}中的鞍点路径。\footnote{The '鞍点路径' (saddle path) is a crucial concept in dynamic systems like the RCK model. The steady state (E) is a saddle point, meaning it has stable and unstable dimensions. - If the economy starts exactly on the saddle path (the stable arm), it will converge to the steady state E. This path represents the unique initial level of consumption $c_0$ (for a given $k_0$) that leads to long-run equilibrium. - If the economy starts off this path, it will diverge away from the steady state. For example, if initial consumption is too high, capital will be depleted, and consumption will eventually fall to zero. If initial consumption is too low, capital will be over-accumulated beyond the optimal path. The assumption is that rational, forward-looking agents will choose $c_0$ to be on this saddle path, ensuring stability. Any deviation would lead to a suboptimal outcome or violate transversality conditions.}
