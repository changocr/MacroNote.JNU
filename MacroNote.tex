...
\begin{theorem}[Hamiltonian函数]
    ...
    \end{align*}\footnote{横截条件经济含义是，在无穷远的未来，资本存量 $k_t$ 的现值（由影子价格 $\mu_t$ 折现）必须为零。这排除了家庭无限期累积债务或资本（庞氏骗局）的可能性，确保其最终消耗掉所有财富或资本价值趋于零，是家庭效用最大化的必要条件之一。如果该条件不成立，意味着家庭在期末仍持有有价值的资本却不消费，这与其效用最大化目标相悖。}
\end{theorem}
...
\begin{align}
  ...
  \end{aligned}
\end{align}
\footnote{此步应用了除法求导法则：对于 $k_t = K_t / (A_t L_t)$，令分子为 $N(t)=K_t$ 分母为 $D(t)=A_t L_t$。则 $\dot{k}_t = \frac{\dot{N}(t)D(t) - N(t)\dot{D}(t)}{[D(t)]^2} = \frac{\dot{K}_t (A_t L_t) - K_t (A_t \dot{L}_t + L_t \dot{A}_t)}{(A_t L_t)^2}$。将其展开并除以 $A_t L_t$ (即乘以 $1/(A_tL_t)$) 便得到该行结果：$\frac{\dot{K}_t}{A_t L_t} - \frac{K_t}{(A_t L_t)^2}(A_t \dot{L}_t + L_t \dot{A}_t)$。}
根据推导结果，当
...
\begin{itemize}
    ...
    \item $\lim_{k \to 0} f^\prime(k)=\infty, \lim_{k \to \infty} f^\prime(k)=0.$\footnote{稻田条件确保了稳定状态的存在性和唯一性。$\lim_{k 	o 0} f'(k)=\infty$ 意味着当资本存量极低时，资本的边际产出极高，投资回报远大于持平投资 $(n+g+\delta)k$，因此经济会自动积累资本，不会停留在 $k=0$。$\lim_{k 	o \infty} f'(k)=0$ 意味着当资本存量极高时，资本的边际产出趋近于零，投资回报最终会低于持平投资，资本积累会停止。这两个条件共同保证 $sf(k)$ 曲线和 $(n+g+\delta)k$ 直线有且仅有一个正的交点 $k^* > 0$。}
\end{itemize}
...
\begin{proof}{图~\ref{ramseyequipoint}中的E点位于$\dot{k} = 0$曲线顶点的左端}
    ...
    \begin{align*}
        k^* < k^{GR}
    \end{align*}\footnote{RCK模型中市场均衡的资本存量 $k^*$ 通常小于黄金律水平 $k^{GR}$（即最大化稳态消费的资本水平），这被称为“动态有效率”。原因是家庭有正的时间偏好率 $ho > 0$ 并且厌恶消费波动（通过 $	heta$ 体现）。这意味着家庭更看重当期消费而非远期消费，并且为了平滑消费路径，不愿意过度储蓄以达到 $k^{GR}$。只有当 $ho + 	heta g = n+g$（在特定条件下，例如 $ho=n$ 且 $	heta=1$ 或 $g=0$）时，$k^*$ 才会等于 $k^{GR}$。$ho - n - (1-	heta)g > 0$ 的条件确保了效用收敛，同时也往往导致 $k^* < k^{GR}$。}
...
资本存量动态学、家庭预算约束以及$k_t$不能为负的条件同时得到满足。
    若我们将初始$k_0$与$c_0$值一一建立联系，则得到图~\ref{ramseyr5}中的鞍点路径。\footnote{鞍点路径是相图中唯一能够收敛到稳态均衡点E的路径。对于给定的初始资本存量 $k_0$，只有唯一的初始消费水平 $c_0$ 能使经济体沿着此路径收敛。若初始消费 $c_0$ 过高（在鞍点路径之上），资本会被过快消耗 ($\dot{k}<0$)，最终 $k$ 趋于0，违反了非负资本的约束或导致效用过低。若初始消费 $c_0$ 过低（在鞍点路径之下），资本会过度积累，消费增长过快，最终违反横截条件（即家庭没有最大化其效用，因为他们在无穷远处留下了有价值的未消费资本）。因此，家庭会理性地选择初始消费 $c_0$ 使其恰好落在鞍点路径上，实现最优消费路径。消费 $c_t$ 作为跳跃变量，可以在初期“跳跃”到鞍点路径所需的值，而资本 $k_t$ 作为状态变量，只能连续变化。}
...
\end{align}\footnote{这是政府在 $t_0$ 时期将资本存量从过高的 $k^*$ 降至黄金律水平 $k^{GR}$ 时，$t_0$ 时期老年人的消费水平。原有资本 $k^*$ 产出为 $f(k^*)$。为了使下一期资本达到 $k^{GR}$，需要的投资是 $(n+g)k^{GR}$ (此处假设 $g=0$，故为 $nk^{GR}$)。因此，可用于消费的是 $f(k^*) - nk^{GR}$。此外，由于资本存量从 $k^*$ 减少到 $k^{GR}$，这部分差额 $(k^* - k^{GR})$ 也可以被当期老年人消费掉（如同“吃掉”一部分存量资本）。所以 $t_0$ 期老年人获得了比正常稳态更高的消费，而未来所有世代的消费都将处于更高的黄金律水平 $f(k^{GR}) - nk^{GR}$。这说明初始的 $k^*$ 处于动态无效率区域。}
...
\begin{definition}[个人效用函数]
    ...
    小写字母$c$与$w$的重新定义为人均消费与人均收入。}\footnote{AK模型的核心特征是资本的边际产出恒定为 $A$。这与之前模型中资本边际产出递减（如Cobb-Douglas函数 $f''(k)<0$）形成对比。$Y_t = AK_t$ 可以被视为一种特殊情况，其中资本不仅包括物质资本，还广义地包括了知识、人力资本等不会出现拥挤或报酬递减的生产要素的线性总和。由于技术进步现在被内生化到这个常数 $A$ 中（或者说，资本的广义定义已经包含了技术要素），我们不再需要外生的技术进步率 $g$。因此，模型中的变量（如 $k, c, y$）通常直接指代人均量，而不是有效人均量。这种设定使得模型可以产生内生的、持续的正增长率。}
...
\subsection{市场均衡}
    ...
    \end{equation}\footnote{中间品厂商是其特定产品种类 $v$ 的垄断生产者。它面临的需求函数由最终品厂商的利润最大化行为决定，即 $x(v,t) = {p^x(v,t)}^{-1/\beta} L$。中间品厂商选择价格 $p^x(v,t)$ 来最大化其利润 $\pi^m = (p^x(v,t) - \varphi)x(v,t)$，其中 $\varphi$ 是生产中间品的边际成本。将需求函数代入利润函数，然后对 $p^x(v,t)$ 求一阶导数并令其为零，即可解出此垄断定价规则。价格高于边际成本 $\varphi$，加价幅度取决于参数 $\beta$（与中间品之间的替代弹性相关）。这个超额利润是创新的回报。}
...
\begin{definition}[市场无套利条件]
    ...
    \end{equation}\footnote{与种类扩张模型类似，这是资产定价的无套利条件，但增加了一项关键的“熊彼特”成分。$V(v,t|q)$ 是当前领先质量产品专利的价值。持有此专利的预期回报包括：当期利润流 $\pi(v,t|q)$ 和专利价值的变动 $\dot{V}(v,t|q)$。然而，与种类扩张不同，现有领先产品面临被更高质量产品“创造性毁灭”的风险。$z_t$ 是（平均）研发成功概率，即下一代更高质量产品出现的瞬时概率。一旦创新成功，现有领先产品的垄断地位和价值 $V(v,t|q)$ 将会消失。因此，持有当前领先专利的预期损失是 $z_t V(v,t|q)$。均衡时，总的预期回报（利润 + 价值变动 - 预期被取代损失）必须等于市场无风险回报 $r_t V(v,t|q)$。整理后即为 $r_t V - \dot{V} = \pi - z_t V$。在BGP上，通常 $\dot{V}=0$，则 $ (r_t+z_t)V = \pi$。这里的 $(r_t+z_t)$ 可以看作是投资于创新活动的有效贴现率，因为它包含了被取代的风险。}
