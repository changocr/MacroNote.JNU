...
\begin{theorem}[Hamiltonian函数]
    ...
    \end{align*}
    两端关于时间$t$求偏导，等式关系仍然成立\footnote{\textcolor{red}{此步骤对时间 $t$ 求全微分。对于左侧 $\ln \mu_t$，其对 $t$ 的导数为 $\frac{d \ln \mu_t}{dt} = \frac{1}{\mu_t} \frac{d\mu_t}{dt} = \frac{\dot{\mu}_t}{\mu_t}$。对于右侧 $-(\rho-n)t + \ln u'(c_t)$，第一项对 $t$ 求导为 $-(\rho-n)$。第二项 $\ln u'(c_t)$ 是 $c_t$ 的函数，$c_t$ 又是 $t$ 的函数，因此根据链式法则，$\frac{d \ln u'(c_t)}{dt} = \frac{d \ln u'(c_t)}{d u'(c_t)} \frac{d u'(c_t)}{d c_t} \frac{dc_t}{dt} = \frac{1}{u'(c_t)} u''(c_t) \dot{c}_t$。}}
    \begin{align*}
    ...
    \end{align*}\footnote{\textcolor{red}{横截条件经济含义是，在无穷远的未来，资本存量 $k_t$ 的现值（由影子价格 $\mu_t$ 折现）必须为零。这排除了家庭无限期累积债务或资本（庞氏骗局）的可能性，确保其最终消耗掉所有财富或资本价值趋于零，是家庭效用最大化的必要条件之一。如果该条件不成立，意味着家庭在期末仍持有有价值的资本却不消费，这与其效用最大化目标相悖。}}
\end{theorem}
...
\begin{align}
  ...
  \end{aligned}
\end{align}
\footnote{\textcolor{red}{此步应用了除法求导法则：对于 $k_t = K_t / (A_t L_t)$，令分子为 $N(t)=K_t$ 分母为 $D(t)=A_t L_t$。则 $\dot{k}_t = \frac{\dot{N}(t)D(t) - N(t)\dot{D}(t)}{[D(t)]^2} = \frac{\dot{K}_t (A_t L_t) - K_t (A_t \dot{L}_t + L_t \dot{A}_t)}{(A_t L_t)^2}$。将其展开并除以 $A_t L_t$ (即乘以 $1/(A_tL_t)$) 便得到该行结果：$\frac{\dot{K}_t}{A_t L_t} - \frac{K_t}{(A_t L_t)^2}(A_t \dot{L}_t + L_t \dot{A}_t)$。}}
根据推导结果，当
...
\begin{itemize}
    ...
    \item $\lim_{k \to 0} f^\prime(k)=\infty, \lim_{k \to \infty} f^\prime(k)=0.$\footnote{\textcolor{red}{稻田条件确保了稳定状态的存在性和唯一性。$\lim_{k 	o 0} f'(k)=\infty$ 意味着当资本存量极低时，资本的边际产出极高，投资回报远大于持平投资 $(n+g+\delta)k$，因此经济会自动积累资本，不会停留在 $k=0$。$\lim_{k 	o \infty} f'(k)=0$ 意味着当资本存量极高时，资本的边际产出趋近于零，投资回报最终会低于持平投资，资本积累会停止。这两个条件共同保证 $sf(k)$ 曲线和 $(n+g+\delta)k$ 直线有且仅有一个正的交点 $k^* > 0$。}}
\end{itemize}
...
由家庭效用最大化,得有效人均消费的动态方程。
        \begin{align}
          \begin{aligned}
            ...
            & \, \mathrm{s.t.} \quad k_0 + \int_{t=0}^\infty e^{-R_t + (n+g)t} (w_t - c_t) dt = 0,\footnote{\textcolor{red}{此为家庭的跨期预算约束的现值形式。$k_0$ 是初始人均资本（财富）。积分项代表未来所有时期的人均储蓄（工资 $w_t$ 减去消费 $c_t$）的现值之和。贴现因子 $e^{-R_t + (n+g)t}$ 中，$R_t = \int_0^t r_s ds$ 是累积市场利率，$e^{-R_t}$ 将未来收入/支出折算回 $t=0$ 时刻的价值。乘以 $e^{(n+g)t}$ 是因为 $w_t$ 和 $c_t$ 是有效人均量，而家庭总人数以 $n$ 增长，技术以 $g$ 增长，将其调整为总量概念再进行跨期加总。约束为零（或大于等于零）意味着家庭的终生支出不能超过其终生收入（包括初始财富）。}}
        ...
        \end{equation}
        对其两端取对数\footnote{\textcolor{red}{从一阶条件 $B e^{-\beta t} c_t^{-\theta} = \lambda e^{-R_t} e^{(n+g)t}$ 到消费欧拉方程的关键步骤是：1. 对两边取自然对数：$\ln B - \beta t - \theta \ln c_t = \ln \lambda - R_t + (n+g)t$。2. 对时间 $t$ 求导：注意 $R_t = \int_0^t r_s ds$，所以 $\dot{R}_t = r_t$。求导后得到 $-\beta - \theta (\dot{c}_t/c_t) = -r_t + (n+g)$。3. 代回 $\beta = \rho - (1-\theta)g - n$ 并整理，即可得到 $\frac{\dot{c}_t}{c_t} = \frac{r_t - \rho - \theta g}{\theta}$。这个方程描述了消费增长率如何取决于市场利率与家庭主观参数的关系。}}
        \begin{align}
            \begin{aligned} 
...
\begin{proof}{图~\ref{ramseyequipoint}中的E点位于$\dot{k} = 0$曲线顶点的左端}
    ...
    \begin{align*}
        k^* < k^{GR}
    \end{align*}\footnote{\textcolor{red}{RCK模型中市场均衡的资本存量 $k^*$ 通常小于黄金律水平 $k^{GR}$（即最大化稳态消费的资本水平），这被称为“动态有效率”。原因是家庭有正的时间偏好率 $\rho > 0$ 并且厌恶消费波动（通过 $\theta$ 体现）。这意味着家庭更看重当期消费而非远期消费，并且为了平滑消费路径，不愿意过度储蓄以达到 $k^{GR}$。只有当 $\rho + \theta g = n+g$（在特定条件下，例如 $\rho=n$ 且 $\theta=1$ 或 $g=0$）时，$k^*$ 才会等于 $k^{GR}$。$\rho - n - (1-\theta)g > 0$ 的条件确保了效用收敛，同时也往往导致 $k^* < k^{GR}$。}}
...
资本存量动态学、家庭预算约束以及$k_t$不能为负的条件同时得到满足。
    若我们将初始$k_0$与$c_0$值一一建立联系，则得到图~\ref{ramseyr5}中的鞍点路径。\footnote{\textcolor{red}{鞍点路径是相图中唯一能够收敛到稳态均衡点E的路径。对于给定的初始资本存量 $k_0$，只有唯一的初始消费水平 $c_0$ 能使经济体沿着此路径收敛。若初始消费 $c_0$ 过高（在鞍点路径之上），资本会被过快消耗 ($\dot{k}<0$)，最终 $k$ 趋于0，违反了非负资本的约束或导致效用过低。若初始消费 $c_0$ 过低（在鞍点路径之下），资本会过度积累，消费增长过快，最终违反横截条件（即家庭没有最大化其效用，因为他们在无穷远处留下了有价值的未消费资本）。因此，家庭会理性地选择初始消费 $c_0$ 使其恰好落在鞍点路径上，实现最优消费路径。消费 $c_t$ 作为跳跃变量，可以在初期“跳跃”到鞍点路径所需的值，而资本 $k_t$ 作为状态变量，只能连续变化。}}
...
\begin{align}
    ...
    \end{aligned}
\end{align}\footnote{\textcolor{red}{此为OLG模型中的资本积累方程。$K_{t+1}$ 是 $t+1$ 期的总资本存量，它等于 $t$ 期年轻人的总储蓄。年轻人的数量是 $L_t$，每位年轻人的有效劳动收入是 $A_t w_t$ (其中 $w_t$ 是有效人均工资)。他们的储蓄率为 $s(r_{t+1})$。所以 $K_{t+1} = L_t \cdot (A_t w_t) \cdot s(r_{t+1})$。为了得到有效人均资本 $k_{t+1} = K_{t+1}/(A_{t+1}L_{t+1})$，我们将上式两边同除以 $A_{t+1}L_{t+1}$。注意到 $A_{t+1} = A_t(1+g)$ 和 $L_{t+1} = L_t(1+n)$。代入并整理即得 $k_{t+1} = \frac{1}{(1+n)(1+g)} s(r_{t+1}) w_t$。第二行是将 $w_t = f(k_t) - k_t f'(k_t)$ (劳动报酬等于其边际产出，在C-D生产函数下等于产出减去资本的总回报) 和 $r_{t+1} = f'(k_{t+1})$ (利率等于资本边际产出) 代入的结果。}}
...
\end{align}\footnote{\textcolor{red}{这是政府在 $t_0$ 时期将资本存量从过高的 $k^*$ 降至黄金律水平 $k^{GR}$ 时，$t_0$ 时期老年人的消费水平。原有资本 $k^*$ 产出为 $f(k^*)$。为了使下一期资本达到 $k^{GR}$，需要的投资是 $(n+g)k^{GR}$ (此处假设 $g=0$，故为 $nk^{GR}$)。因此，可用于消费的是 $f(k^*) - nk^{GR}$。此外，由于资本存量从 $k^*$ 减少到 $k^{GR}$，这部分差额 $(k^* - k^{GR})$ 也可以被当期老年人消费掉（如同“吃掉”一部分存量资本）。所以 $t_0$ 期老年人获得了比正常稳态更高的消费，而未来所有世代的消费都将处于更高的黄金律水平 $f(k^{GR}) - nk^{GR}$。这说明初始的 $k^*$ 处于动态无效率区域。}}
...
\begin{definition}[个人效用函数]
    ...
    小写字母$c$与$w$的重新定义为人均消费与人均收入。}\footnote{\textcolor{red}{AK模型的核心特征是资本的边际产出恒定为 $A$。这与之前模型中资本边际产出递减（如Cobb-Douglas函数 $f''(k)<0$）形成对比。$Y_t = AK_t$ 可以被视为一种特殊情况，其中资本不仅包括物质资本，还广义地包括了知识、人力资本等不会出现拥挤或报酬递减的生产要素的线性总和。由于技术进步现在被内生化到这个常数 $A$ 中（或者说，资本的广义定义已经包含了技术要素），我们不再需要外生的技术进步率 $g$。因此，模型中的变量（如 $k, c, y$）通常直接指代人均量，而不是有效人均量。这种设定使得模型可以产生内生的、持续的正增长率。}}
...
\begin{corollary}
    ...
    \end{aligned}
    \end{align}
    可观察到企业资本边际产出$R^*$小于社会资本边际产出$R^s$，即存在外部性问题。\footnote{\textcolor{red}{单个企业在决定其最优资本投入 $K_i$ 时，将其自身的技术水平 $A_i$ 视为外生给定的，并且认为其自身投资对总体技术水平 $A = BK$ (其中 $K = \sum K_i$) 的影响微乎其微。因此，它只考虑其投资对自身产出的直接贡献，即 $R^* = \partial Y_i / \partial K_i$ (在 $A$ 被视为固定的情况下)。然而，当所有企业都增加投资时，总资本 $K$ 上升，从而提高了整体技术水平 $A=BK$。这个整体技术水平的提升使所有企业受益，但单个企业在做决策时并未将这部分“知识溢出”或“学习效应”内化。社会计划者则会考虑到这种正外部性，即资本积累不仅增加了个别企业的产出，还通过提升 $A$ 增加了所有企业的产出。因此，资本的社会边际产出 $R^s$ (考虑到 $K$ 增加对 $A$ 的影响) 高于企业感知的私人边际产出 $R^*$。这种差异导致市场均衡下的投资水平和增长率低于社会最优水平。}}
...
\begin{definition}[市场无套利条件]
    ...
    \end{equation}\footnote{\textcolor{red}{此为资产定价的无套利条件。$V(v,t)$ 是一个已存在的中间品种类 $v$ (或其专利) 的市场价值。持有这项资产的回报有两种：一是资产本身的价值增值（或贬值）$\dot{V}(v,t)$；二是该资产带来的即期利润流 $\pi(v,t)$（即垄断利润）。在均衡状态下，持有这项资产的总回报率必须等于市场利率 $r_t$。换句话说，$\pi(v,t) + \dot{V}(v,t)$ (利润加上资本利得) 必须等于将 $V(v,t)$ 投资于其他任何资产所能获得的无风险回报 $r_t V(v,t)$。如果前者大于后者，则所有人都想持有该专利，推高其价格 $V$ 直至等式成立；反之亦然。在BGP上，新产品价值 $V$ 通常是恒定的 ($\dot{V}=0$)，因此 $r_t V = \pi(v,t)$。}}
...
\subsection{市场均衡}
    ...
    \end{equation}\footnote{\textcolor{red}{中间品厂商是其特定产品种类 $v$ 的垄断生产者。它面临的需求函数由最终品厂商的利润最大化行为决定，即 $x(v,t) = {p^x(v,t)}^{-1/\beta} L$。中间品厂商选择价格 $p^x(v,t)$ 来最大化其利润 $\pi^m = (p^x(v,t) - \varphi)x(v,t)$，其中 $\varphi$ 是生产中间品的边际成本。将需求函数代入利润函数，然后对 $p^x(v,t)$ 求一阶导数并令其为零，即可解出此垄断定价规则。价格高于边际成本 $\varphi$，加价幅度取决于参数 $\beta$（与中间品之间的替代弹性相关）。这个超额利润是创新的回报。}}
...
\begin{definition}[市场无套利条件]
    ...
    \end{equation}\footnote{\textcolor{red}{与种类扩张模型类似，这是资产定价的无套利条件，但增加了一项关键的“熊彼特”成分。$V(v,t|q)$ 是当前领先质量产品专利的价值。持有此专利的预期回报包括：当期利润流 $\pi(v,t|q)$ 和专利价值的变动 $\dot{V}(v,t|q)$。然而，与种类扩张不同，现有领先产品面临被更高质量产品“创造性毁灭”的风险。$z_t$ 是（平均）研发成功概率，即下一代更高质量产品出现的瞬时概率。一旦创新成功，现有领先产品的垄断地位和价值 $V(v,t|q)$ 将会消失。因此，持有当前领先专利的预期损失是 $z_t V(v,t|q)$。均衡时，总的预期回报（利润 + 价值变动 - 预期被取代损失）必须等于市场无风险回报 $r_t V(v,t|q)$。整理后即为 $r_t V - \dot{V} = \pi - z_t V$。在BGP上，通常 $\dot{V}=0$，则 $ (r_t+z_t)V = \pi$。这里的 $(r_t+z_t)$ 可以看作是投资于创新活动的有效贴现率，因为它包含了被取代的风险。}}
...
\subsection{市场均衡}
    ...
    \end{align}\footnote{\textcolor{red}{此BGP增长率是通过联立几个关键方程得到的：1. 消费欧拉方程: $g_C = (r_t - \rho)/\theta$。2. 资本/资产市场无套利条件 (考虑创造性毁灭): $r_t + z_t = \lambda \eta \beta L$ (由式 ef{rzqual}，这里假设BGP下 $\dot{V}=0$, $\pi/V = \lambda \eta \beta L$ )。3. 消费增长与质量改进挂钩: $g_C = g_Q$ (由式 ef{gqual})。4. 质量改进率与研发强度关系: $g_Q = (\lambda-1)z_t$ (由式 ef{gqqual})。从(1) $r_t = \theta g_C + \rho$。从(4) $z_t = g_Q/(\lambda-1) = g_C/(\lambda-1)$ (用(3))。将 $r_t$ 和 $z_t$ 代入(2): $(\theta g_C + \rho) + g_C/(\lambda-1) = \lambda \eta \beta L$。整理此式，解出 $g_C$ 即可得到该表达式。这个增长率反映了创新激励（$\lambda, \eta, \beta, L$）、家庭偏好（$\rho, \theta$）和创新幅度（$\lambda-1$）之间的复杂互动。}}
